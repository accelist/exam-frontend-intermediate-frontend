...
const server = Proxy.createProxyServer({
  target: AppSettings.current.backendApiHostCustom,
...
server.on("proxyReq", (proxyReq, req) => {
  ...
  console.log(
    ...
    req.url,
    "-->",
    AppSettings.current.backendApiHostCustom + urlRewrite
  );
});
...
const apiGateway = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  res.on("finish", () => {
    ...
    console.log(
      `API Proxy: Finished ${res.req.url} in ${endTime - startTime}ms `
    );
  });
};
